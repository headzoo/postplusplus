...
export const ContextDocumentAssignmentPicker: FC<{
  ...
  const rows = useMemo(() => {
    ...
    const knownById = new Map(
      knownDocuments.map((document) => [document.id, document])
    );
    const ids = new Set([
                ...
                >
                  ...
                  <div className="flex flex-col gap-[4px] min-w-0 flex-1">
                    <div className="flex items-center gap-[8px] flex-wrap">
                      <span className="text-[14px] font-[600] truncate">
                        {row.name}
                      </span>
                      {row.isLarge && (
                    ...
                    {!row.stale && (
                      <div className="text-[12px] opacity-70 flex flex-wrap gap-x-[10px] gap-y-[2px]">
                        <span>
                          {t('size', 'Size')}:{' '}
                          {formatContextDocumentSize(row.fileSize)}
                        </span>
                        <span>
                          {t('updated', 'Updated')}:{' '}
    ...
    return (
      <div className="flex flex-wrap items-center gap-[6px]">
        {visible.map((document) => (
          <span
            key={document.id}
            className="inline-flex max-w-[180px] truncate text-[11px] px-[7px] py-[2px] rounded-full border border-newBorder bg-newBgColorInner text-textItemBlur"
            title={document.name}
          >
            {document.name}
          </span>
