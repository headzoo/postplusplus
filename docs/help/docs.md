...
**Docs** (page title **Context documents**) is your organization’s Markdown library for AI. Open it from **Docs** in the main navigation (`/context-documents`).

These files are **not** this Help panel. Help articles answer “how do I use Post++?” Docs feed brand voice, briefs, and agent procedures into Pipelines and Agent.

## What Docs are

Two roles share the same library:

1. **Standard documents** (for example `BRANDING.md`) — attach to [Pipelines](/help/pipelines#context-documents) so agents can read them when drafting.
...
- **Replace** — upload a new file; the document id stays the same so Pipeline links keep working.
- **Delete** — confirm **Delete document?** Removing a document also detaches it from Pipelines. **Queued posts are not affected.**

Cards can show size, **Large**, **Skill · /{slug}**, or **Skill conflict · /{slug}**.
...
- **Selected: N**, **Clear all**, and **Manage library** (opens Docs).
